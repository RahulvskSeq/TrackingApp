...
// import { LayoutDashboard, Users, TrendingUp, Settings, LogOut, Bell, GitCompare, Menu, RefreshCw, Map } from 'lucide-react';
// import { DEFAULT_USERS, MO, CURRENT_MONTH_IDX } from './constants';
// import { pct, spct, pclr, uid, isoNow, storage, parseCSV, fetchCSV } from './utils';
...
// const VALID_SCREENS = ['overview','dealers','monthly','compare','map','followups','admin'];
...
//       if(!s.url)continue;
//       try{ const csv=await fetchCSV(s.url);const rows=parseCSV(csv,s.id);if(!rows.length)throw new Error('0 rows');live.push(...rows); }
//       catch(e){ errs.push(s.name+': '+e.message); }
//     }
//     if(live.length)setDealers(live);
...
//           {/* ── Topbar ── */}
//           <div id="topbar">
//             <button onClick={()=>setSidebarOpen(s=>!s)} className="btn" style={{padding:'5px 9px',border:'none',background:'none',color:'var(--t3)'}}><Menu size={16}/></button>
//             <div style={{fontFamily:'"JetBrains Mono",monospace',fontSize:11,color:'var(--acc)',letterSpacing:3,fontWeight:500}}>▸ SALES TRACKER PRO</div>

//             {/* Territory bar */}
//             <div style={{display:'flex',alignItems:'center',gap:12,marginLeft:16,padding:'4px 14px',background:'var(--bg2)',borderRadius:8,border:'1px solid var(--b2)',fontSize:11}}>
//               <span style={{color:'var(--t3)',fontSize:10,textTransform:'uppercase',letterSpacing:'.1em'}}>{MO[selectedMonthIdx]}</span>
...
//               <span style={{color:'#34d399'}} title="Active dealers">●{activeCount}</span>
//               <span style={{color:'#fbbf24'}} title="Inactive dealers">◑{inactiveCount}</span>
//               <span style={{color:'#f87171'}} title="Dead dealers">○{deadCount}</span>
//               <span style={{color:'var(--t3)',fontSize:10}}>{myDealers.length} total</span>
//             </div>

//             <div className="spacer"/>
//             {lastSync&&<div style={{fontSize:11,color:'var(--t3)',display:'flex',alignItems:'center',gap:5}}>
//               <span style={{width:6,height:6,borderRadius:'50%',background:syncErrs.length?'var(--red)':'var(--grn)'}}/>
//               {lastSync}{syncErrs.length>0&&` (${syncErrs.length} err)`}
//             </div>}
//             <button onClick={syncSheets} disabled={syncing} className="btn" style={{fontSize:11,display:'flex',alignItems:'center',gap:4,padding:'4px 10px'}}>
//               <RefreshCw size={11} className={syncing?'spin':''}/> {syncing?'Syncing':'Sync'}
//             </button>
...
//             <div className="hmob">
//               <div style={{fontSize:12,fontWeight:600,color:'var(--t1)',lineHeight:1.1}}>{currentUser.name}</div>
//               <div style={{fontSize:10,color:'var(--t3)'}}>{currentUser.role==='admin'?'Administrator':'Salesman'}</div>
//             </div>
//             <button onClick={handleLogout} className="btn" style={{padding:'4px 10px',fontSize:11,color:'var(--t3)',display:'flex',alignItems:'center',gap:4}}>
//               <LogOut size={11}/><span className="hmob">Sign out</span>
//             </button>
//           </div>
...
//             <div id="sidebar" className={sidebarOpen?'':'closed'}>
...
//                   {screen==='admin'&&currentUser.role==='admin'&&<AdminPanel dealers={dealers} users={users} setUsers={setUsers} setShowUM={setShowUM} onSync={syncSheets} syncing={syncing} lastSync={lastSync} syncErrs={syncErrs} onNavigate={navigate} onOpenDealer={setEditingId}/>}
//                 </>
//               )}
//             </div>
//           </div>
...
//         {editing&&<DealerModal dealer={editing} users={users} currentUser={currentUser} onSave={saveDealer} onDelete={deleteDealer} onClose={()=>setEditingId(null)} notes={notes} onAddNote={addNote} onUpdateNote={updateNote} onDeleteNote={deleteNote} onLog={addLog}/>}
    ...
    for(const s of Object.values(users).filter(u=>u.role==='salesman')){
      ...
      if(s.url){
        try{
          ...
          const rows=parseCSV(csv,s.id);
          if(!rows.length) throw new Error('0 rows');
          live.push(...rows);
        } catch(e){ errs.push(s.name+' Sheet1: '+e.message); }
      }
      // Fetch Sheet 2 (url2) — merge with Sheet 1 dealers by name
      if(s.url2){
        try{
          const csv2=await fetchCSV(s.url2);
          const rows2=parseCSV(csv2,s.id);
